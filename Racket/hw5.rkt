...
(define (racketlist->mupllist racketlist)
  (letrec ([transfer (lambda (racketlist mupllist)
                    (if (null? racketlist)
                        mupllist
                        (apair (car racketlist) (transfer (cdr racketlist) mupllist))))])
    (transfer racketlist (aunit))))
  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? v1)
               (let ([v3 (fun-nameopt (closure-fun v1))]
